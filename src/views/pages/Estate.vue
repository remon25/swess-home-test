<template>
  <div class="">
    ...
    <div v-if="!isLoad">
        ...
        <Photoswipe>
          <v-row>
            <v-col cols="12" md="12" lg="4">
              <img :src="img_baseUrl + estateImages[1].url" v-pswp="img_baseUrl + estateImages[1].url"
                v-if="estateImages[1]" class="d-border-radius img rounded-b-0"
                :class="$i18n.locale == 'ar' ? 'rounded-l-0' : 'rounded-r-0'" width="100%" height="233px" cover />

              <img :src="img_baseUrl + estateImages[2].url" v-pswp="img_baseUrl + estateImages[2].url"
                v-if="estateImages[2]" class="d-border-radius mt-3 img"
                :class="$i18n.locale == 'ar' ? 'rounded-l-0' : 'rounded-r-0'" width="100%" height="233px" />
            </v-col>

            <v-col cols="12" md="12" lg="8">
              <div class="d-p-relative">

                <img :src="img_baseUrl + estateImages[0].url" v-pswp="img_baseUrl + estateImages[0].url"
                  v-if="estateImages[0]" class="d-border-radius img "
                  :class="$i18n.locale == 'ar' ? 'rounded-r-0' : 'rounded-l-0'" width="100%" height="485px" />
              ...
              <div v-for="(item, i) in estateImages" :key="i" style="display: none">
                <img v-pswp="img_baseUrl + estateImages[i + 3].url" v-if="estateImages[i + 3]"
      ...
      <v-container>
        <v-row>
          ...
          <v-col cols="12" md="12" lg="4">
            <v-card width="412px" class="d-border-radius office-box" height="411px"
              :class="item.contract_id == 4 ? 'contract-gold' : item.contract_id == 3 ? 'contract-pron' : item.contract_id == 2 ? 'contract-selver' : 'contract-free'">
              <p class="subtitle1 pb-4" style="padding-top: 19px">
                {{ $t("real_estate_office") }}
              </p>
              <v-row>
                <Photoswipe>
                  <img width="145px" height="145px" :src="img_baseUrl + item.office.logo"
                    v-pswp="img_baseUrl + item.office.logo" style="border-radius: 50%" />
                </Photoswipe>

                <div class="subtitle1 mt-md-15" style="margin-right: 32px">
                  <p>{{ item.office.name }}</p>
                  <a :href="'tel:' + item.office.mobile" class="body1" style="text-decoration:none">
                    {{ $t("call") }}
                    <img src="/icons/phone.svg" class="mx-3" />
                  </a>

                  <div>
                    <a :href="'https://wa.me/' + item.office.mobile" class="body1" style="text-decoration:none">
                      {{ $t("whatsapp") }}
              ...
              <router-link :to="'/office/' + item.office.id" class="subtitle0 d-text-primary more-office">{{ $t('More')
              }}
              </router-link>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="12" lg="8">
            <div>
              <div class="h6  my-5">
                ...
                <div class="d-inline mx-2 " :class="$i18n.locale == 'ar' ? 'mr-15' : 'ml-15'" @click="save(item.id)"
                  style="cursor: pointer">
                  <v-icon v-if="is_saved" width="25.19" class="d-text-primary">mdi-bookmark</v-icon>
                  <v-icon v-else width="25.19" class="d-text-primary">mdi-bookmark-outline</v-icon>
                </div>
              ...
              <p class="h6 d-text-dark_gray">
                {{ $i18n.locale == "ar" ? item.estate_type.name_ar.split("|")[1] :
                  item.estate_type.name_en.split("|")[1]
                }},

                {{ $i18n.locale == "ar" ? item.estate_offer_type.name_ar : item.estate_offer_type.name_en }},
                {{
                  $i18n.locale == "ar"
              ...
              <p class="h5 d-text-dark_gray">
                {{ item.price.replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") + ($i18n.locale == 'ar' ? " ل.س" : " S.P") }}
              </p>
              ...
              <div class="subtitle0 mt-4 d-text-dark_gray">
                ...
                <div v-if="item.interior_status">
                  ...
                  <span style="display: inline-block;">
                    <img style="vertical-align:middle" src="/icons/icon-bed.svg" class="mx-2" />
                  </span>
                  <span style="display: inline-block;">
                ...
                <div v-if="item.area">
                  <span style="display: inline-block;">
                    <img style="vertical-align:middle" src="/icons/star.svg" class="mx-2" />
                  </span>
                  <span style="display: inline-block;">
              ...
              <div class="my-5">
                ...
                <v-row>
                  <v-col cols="12" sm="6" md="4" v-if="item.estate_offer_type"><img class="d-inline"
                      src="/icons/point.svg" /> <span class='subtitle2'>{{ $t("OfferType") + ' : ' }}</span>
                    {{ $i18n.locale == "ar" ? item.estate_offer_type.name_ar : item.estate_offer_type.name_en }}</v-col>
                  <v-col cols="12" sm="6" md="4" v-if="item.estate_type"><img class="d-inline" src="/icons/point.svg" />
                    <span class='subtitle2'>{{ $t("EstateType") + ' : ' }}</span>
                    {{ $i18n.locale == "ar" ? item.estate_type.name_ar.split("|")[1] :
                      item.estate_type.name_en.split("|")[1]
                    }}
                  </v-col>
                  <v-col cols="12" sm="6" md="4" v-if="item.floor"><img class="d-inline" src="/icons/point.svg" /> <span
                      class='subtitle2'>{{ $t("floor") + ' : ' }}</span>
                    {{ item.floor }}</v-col>
                </v-row>
              </div>
              ...
              <div class="my-5" v-if="floorImages.length > 0">
                ...
                <v-row>
                  <v-col cols="12">
                    <v-img @click="open_img(indexFloor, 'floor_plan')" v-if="floorImages[indexFloor]" width="856px"
                      height="485px" class="d-border-radius img " :src="img_baseUrl + floorImages[indexFloor].url">
                    </v-img>
                  </v-col>
                  <v-divider class="my-5"></v-divider>
                  <v-col cols="3" v-for="(item, i) in floorImages" :key="i">
                    <v-img @click="indexFloor = i" v-if="item" class="d-border-radius img "
                      :src="img_baseUrl + item.url"></v-img>
                  </v-col>
                </v-row>
              </div>
              ...
              <div class="my-5">
                ...
                <v-row v-if="item.nearby_places">
                  ...
                  <v-col cols="6" md="4" v-for="(place, i) in item.nearby_places.split('|')" :key="i">
              ...
              <div v-if="!(visibleItem.length == 0)">
                <EstateCard class="my-10" v-for="(estate, i) in visibleItem" :key="i" :area="estate.area"
                  ...
                  :internalstate="estate.interior_status" :images="estate.images" :office="estate.office"
                  :price="estate.price" :location="estate.location.locations[0].name" :description="estate.description"
                  :date="estate.created_at" :phone="estate.office.mobile" :logo="estate.office.logo"
                  :contract="estate.office" :id="estate.id"></EstateCard>

                <v-pagination dir="ltr" circle v-model="page" color="#2170B4" :length="Math.ceil(Estate.length / perPage)"
    ...
    <div v-else>
      <v-container class="">
        <v-row>
          ...
          <v-col cols="12" md="12" lg="4">
            <v-skeleton-loader type="list-item-avatar, article, actions"></v-skeleton-loader>
          </v-col>
          <v-col cols="12" md="12" lg="8">
            <v-skeleton-loader type=" divider, list-item-three-line, card-heading, image, actions"></v-skeleton-loader>
          </v-col>
        </v-row>
      </v-container>
    </div>
    ...
    <v-dialog v-model="dialog_img" max-width="800">
      <Carousal :images="img_selected" :model="model" @closeDialog="dialog_img = false"></Carousal>
    </v-dialog>
